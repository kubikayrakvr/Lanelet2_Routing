...
lanelet::routing::LaneletPath overtakePath(lanelet::LaneletMapPtr map_, lanelet::routing::LaneletPath current_path);
nav_msgs::Path drawPath(lanelet::routing::LaneletPath current_path);
lanelet::routing::LaneletPath dockingPath(lanelet::LaneletMapPtr map_, lanelet::routing::LaneletPath current_path);
...
void setPath(ros::NodeHandle nh_){
    ...
    lanelet::routing::LaneletPath current_path;
    ...
    while (ros::ok()) {
        if (map_->laneletLayer.exists(start_id)){
            if(goal_id != current_goal || start_id != current_start){
                current_path = findPath(map_, current_start, current_overtake);
                current_path = dockingPath(map_, current_path);
                current_goal = goal_id;
                current_start = start_id;
            }
            if(overtake_id != current_overtake){
                if(map_->laneletLayer.exists(overtake_id) && overtake_id != 0){
                    current_path = overtakePath(map_, current_path);
                    current_overtake = overtake_id;
                }
            }
            nav_msgs::Path path = drawPath(current_path);
...
lanelet::routing::LaneletPath findPath(lanelet::LaneletMapPtr map_, const lanelet::Id &current_start, const lanelet::Id &current_overtake){
    ...
    lanelet::routing::LaneletPath current_path = route->shortestPath();
    return current_path;
}

lanelet::routing::LaneletPath dockingPath(lanelet::LaneletMapPtr map_, lanelet::routing::LaneletPath current_path) {
    std::vector<lanelet::ConstLanelet> path_vector;
    
    for(const auto& lanelet : current_path) {
        ...
        for(const auto& area : map_->areaLayer) {
            if(lanelet::geometry::overlaps2d(area, lanelet) && area.id() != alreadyDocked){
                ...
                path_vector.push_back(lanelet::Lanelet(lanelet::utils::getId(), leftBound, rightBound).invert());
                break;
            }
        }
        if (!needsDocking) {
            path_vector.push_back(lanelet);
        }
    }
    
    return lanelet::routing::LaneletPath(path_vector);
}

lanelet::routing::LaneletPath overtakePath(lanelet::LaneletMapPtr map_, lanelet::routing::LaneletPath current_path){
    if(map_->laneletLayer.exists(overtake_id) && overtake_id != 0){
        lanelet::ConstLanelet overtake_lanelet = map_->laneletLayer.get(overtake_id);
        auto left_lanelet = map_->laneletLayer.findUsages(overtake_lanelet.leftBound().invert());
        if(!left_lanelet.empty()){
            std::vector<lanelet::ConstLanelet> path_vector;
            for(const auto& lanelet : current_path){
                if(lanelet.id() != overtake_id){
                    ...
                    lanelet::Lanelet inverted = left_lanelet[0].invert();
                    path_vector.push_back(inverted);
                }
            }
            lanelet::routing::LaneletPath overtakePath(path_vector);
            current_path = overtakePath;
        }
        else{
