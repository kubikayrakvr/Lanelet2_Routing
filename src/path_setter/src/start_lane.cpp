...
void findStart(ros::NodeHandle nh_) {
    ...
    while(ros::ok()){
        if(receivedStart){
            current_lane_id=nearestLane(map_, start_msg);
            if(startInside){
                ROS_INFO("Chosen Start Lanelet Id: %ld", current_lane_id);
            }
            else{
                ROS_INFO("Nearest Chosen Start Lanelet Id: %ld", current_lane_id);
            }
            receivedStart=false;
            startInside=false;
        }
...
lanelet::Id nearestLane(lanelet::LaneletMapPtr map_, const geometry_msgs::PoseWithCovarianceStamped& start_msg){
    lanelet::BasicPoint3d received_point(start_msg.pose.pose.position.x, start_msg.pose.pose.position.y, start_msg.pose.pose.position.z);
    double distance;
...
lanelet::Id findDock(lanelet::LaneletMapPtr map_, const geometry_msgs::PoseWithCovarianceStamped& start_msg){
    lanelet::BasicPoint2d received_point(start_msg.pose.pose.position.x, start_msg.pose.pose.position.y);
    auto nearest_dock = lanelet::geometry::findNearest(map_->areaLayer, received_point, 1);
    return nearest_dock[0].second.id();
}